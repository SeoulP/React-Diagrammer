...
export default function Toolbar({addComponent, clearTree}: Props) {
    ...
    const [visibleLines, setVisibleLines] = useLocalStorage('Lines', false)
    ...
    return (
        <div id="toolbar" className="fixed bottom-0 flex h-12 w-full justify-between bg-slate-300 shadow shadow-slate-500">
            <div id={'attribute-settings'} className={'max-md:text-md flex flex-row items-center gap-4 px-4 text-xl font-medium text-slate-500 max-md:gap-2 max-sm:text-sm'}>
                ...
                <label className={"flex h-8 gap-2 items-center justify-center cursor-pointer max-md:mb-0 max-md:items-end max-md:flex-row-reverse"}>
                    <span className="ms-3 font-medium text-xl max-md:text-sm max-md:-ml-1 max-md:-mb-1 text-gray-500">ShowLines: </span>
                    <input type={"checkbox"} className={"sr-only peer"} checked={visibleLines} onChange={(e) => {
                                    setVisibleLines(e.target.checked)
                                }}/>
                <div className="relative top-0.5 w-8 h-4 bg-slate-400 rounded-full peer peer-checked:after:translate-x-[110%] rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[3px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:bg-blue-600"></div>
            </label>
            ...
            <div className={'mr-4 flex flex-row items-center gap-4'}>
                ...
                <button className="flex flex-row gap-1 px-2 items-center relative rounded-full py-2 text-slate-50"
                        onClick={() => addComponent(null)}>
<svg className="w-12 h-12 -mr-4 hover:fill-green-600 fill-green-500" xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="-3 -3 30 30">
  <g>
    <path d="m12 2a10 10 0 1 0 10 10 10 10 0 0 0 -10-10zm0 18a8 8 0 1 1 8-8 8 8 0 0 1 -8 8z"/>
    <path d="m15 11h-2v-2a1 1 0 0 0 -2 0v2h-2a1 1 0 0 0 0 2h2v2a1 1 0 0 0 2 0v-2h2a1 1 0 0 0 0-2z"/>
  </g>
</svg>
                </button>
                
                <button className={""} onClick={() => clearTree()}>
                    <svg className={"fill-red-500 w-6 h-6 hover:fill-red-600 hover:cursor-pointer"} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
                        <path d="M135.2 17.7C140.6 6.8 151.7 0 163.8 0L284.2 0c12.1 0 23.2 6.8 28.6 17.7L320 32l96 0c17.7 0 32 14.3 32 32s-14.3 32-32 32L32 96C14.3 96 0 81.7 0 64S14.3 32 32 32l96 0 7.2-14.3zM32 128l384 0 0 320c0 35.3-28.7 64-64 64L96 512c-35.3 0-64-28.7-64-64l0-320zm96 64c-8.8 0-16 7.2-16 16l0 224c0 8.8 7.2 16 16 16s16-7.2 16-16l0-224c0-8.8-7.2-16-16-16zm96 0c-8.8 0-16 7.2-16 16l0 224c0 8.8 7.2 16 16 16s16-7.2 16-16l0-224c0-8.8-7.2-16-16-16zm96 0c-8.8 0-16 7.2-16 16l0 224c0 8.8 7.2 16 16 16s16-7.2 16-16l0-224c0-8.8-7.2-16-16-16z"/></svg>
                </button>

                <button className={'flex items-center justify-center rounded-full bg-slate-50 shadow shadow-slate-500 hover:bg-slate-200'}
                        onClick={() => {
